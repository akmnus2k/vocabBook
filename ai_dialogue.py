...
# 自定义场景的提示词：围绕用户给的场景，生成一段更完整的对话
SCENE_PROMPT = """你是美国社区诊所里一位说话随和的物理治疗师。\
请围绕这个具体场景「{scene}」，用英文单词「{word}」（中文含义：{zh}）\
写 5 句你在这个场景下对患者说的日常口语，像一段真实的对话那样连贯。

要求：
1. 每句自然用到 {word}（可用常见变形），语法正确
2. 口语自然、像当面聊天，长短都行（可以是完整的一两句话，不用刻意压短），多用缩写
3. 五句要贴合「{scene}」这个场景，并且尽量前后连贯，像一段完整的问诊/指导过程
4. 中文翻译也要口语化，像当面说话
...
# glm-4-flashx 比 glm-4-flash 快近一半、同样免费，用它加速生成
MODEL = "glm-4-flashx"
...
def generate_scene(word: str, zh: str, scene: str, api_key: str, n: int = 5):
